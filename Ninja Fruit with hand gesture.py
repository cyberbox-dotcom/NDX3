...
def reset_game():
    global Score, Lives, Difficulty_level, game_Over, Fruits, Speed, Spawn_Rate
    Score = 0
    Lives = 15
    Difficulty_level = 1
    game_Over = False
...
while (cap.isOpened()):
    ...
    if not (game_Over):
        ...
        Fruits.clear()
        cv2.putText(img, "Press 'C' to Restart", (int(w * 0.1), int(h * 0.7)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
